...
fixture`TC_E2E_UC2_022 - Creating send to send journey and posting message - With one stage - Invalid URL`
  .page(baseUrl)
  .beforeEach(async (t) => {
  ...
  ("TC_E2E_UC2_022-part1  Validating user is able to add the Journey successfully, Direction = SEND and Next direction = SEND", async (t) => {
  ...
  ("TC_E2E_UC2_022-part2: Posting message using send to send journey",  async (t) => {
  ...
  fixture`TC_E2E_UC2_023 - Creating send to receive and receive to offRampEndpoint (JMS.SOLACE.SuccessQueue) journey and posting message - With one stage - Invalid URL`
  .page(baseUrl)
  .beforeEach(async (t) => {
    await LoginPage.logInSuccessful();
  });
  test //part1
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_023-part1: Validate user is able to create a Journey, send to receive",
  async (t) => {
  ...
  test //part2
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_023-part2: Validate user is able to create a Journey, receive to OffRampEndpoint (JMS.SOLACE.SuccessQueue)",
    async (t) => {
...
test //part3
  .meta({ e2e: 'regression', type1: 'usecaseE2E' })
    ("TC_E2E_UC2_023-part3: Posting message using SEND -> RECEIVE, RECEIVE -> OffRamp (JMS.SOLACE.SuccessQueue)",
      async (t) => {
        try {
          // Update the subSource and messageuniqueid in requestBody
...
fixture`TC_E2E_UC2_029 - Creating send to receive and receive to offRampEndpoint (JMS.SOLACE.SuccessQueue) journey and posting message - With one valid and invalid stage URL`
  .page(baseUrl)
  .beforeEach(async (t) => {
    await LoginPage.logInSuccessful();
  });
  test //part1
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_029-part1: Validate user is able to create a Journey, send to receive",
  async (t) => {
    // send to receive journey
    await dashBoardPage.selectMember(member);
    await dashBoardPage.navigateToNetworkSetupPage();
  ...
  test //part2
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_029-part2: Validate user is able to create a Journey, receive to OffRampEndpoint (JMS.SOLACE.SuccessQueue)",
    async (t) => {
      const { participant, endpoint } = ENV_CONFIG[ENV_NAME];
    // receive to offramp endpoint (JMS.SOLACE.SuccessQueue) journey
    await dashBoardPage.selectMember(receiverAsMember);
...
test //part3
  .meta({ e2e: 'regression', type1: 'usecaseE2E' })
    ("TC_E2E_UC2_029-part3: Posting message using SEND -> RECEIVE, RECEIVE -> OffRamp (JMS.SOLACE.SuccessQueue)",
      async (t) => {
        try {
          // Update the subSource and messageuniqueid in requestBody
            requestBody.metadata.messageuniqueid = "sendToReceiveToJMSSuccessQ_testcafe_" + Date.now();
