...
fixture`TC_E2E_UC2_016 - Creating send to send journey and posting message - With two stages`
  .page(baseUrl)
  .beforeEach(async (t) => {
  ...
  ("TC_E2E_UC2_016-part1  Validating user is able to add the Journey successfully, Direction = SEND and Next direction = SEND", async (t) => {
  ...
  ("TC_E2E_UC2_016-part2: Posting message using send to send journey",  async (t) => {
  ...
  fixture`TC_E2E_UC2_017 - Creating send to receive and receive to offRampEndpoint (JMS.SOLACE.SuccessQueue) journey and posting message - With two stages`
  .page(baseUrl)
  .beforeEach(async (t) => {
    await LoginPage.logInSuccessful();
  });
  test //part1
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_017-part1: Validate user is able to create a Journey, send to receive",
  async (t) => {
  ...
  test //part2
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_017-part2: Validate user is able to create a Journey, receive to OffRampEndpoint (JMS.SOLACE.SuccessQueue)",
    async (t) => {
  ...
  test //part3
  .meta({ e2e: 'regression', type1: 'usecaseE2E' })
    ("TC_E2E_UC2_017-part3: Posting message using SEND -> RECEIVE, RECEIVE -> OffRamp (JMS.SOLACE.SuccessQueue)",
      async (t) => {
        try {
          // Update the subSource and messageuniqueid in requestBody
  ...
  fixture`TC_E2E_UC2_018 - Creating send to receive and receive to offRampEndpoint (JMS.SOLACE.FailureQueue) journey and posting message - With two stages`
  .page(baseUrl)
  .beforeEach(async (t) => {
    await LoginPage.logInSuccessful();
  });
  test //part1
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_018-part1: Validate user is able to create a Journey, send to receive",
  async (t) => {
    // send to receive journey
    await dashBoardPage.selectMember(member);
    await dashBoardPage.navigateToNetworkSetupPage();
  ...
  test //part2
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_018-part2: Validate user is able to create a Journey, receive to OffRampEndpoint (JMS.SOLACE.FailureQueue)",
    async (t) => {
  ...
  test //part3
  .meta({ e2e: 'regression', type1: 'usecaseE2E' })
    ("TC_E2E_UC2_018-part3: Posting message using SEND -> RECEIVE, RECEIVE -> OffRamp (JMS.SOLACE.FailureQueue)",
      async (t) => {
        try {
          // Update the subSource and messageuniqueid in requestBody
  ...
  fixture`TC_E2E_UC2_019 - Creating send to receive and receive to receive journey and posting message - With two stages`
  .page(baseUrl)
  .beforeEach(async (t) => {
    await LoginPage.logInSuccessful();
  });
  test //part1
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_019-part1: Validate user is able to create a Journey, send to receive",
  async (t) => {
    // send to receive journey
    await dashBoardPage.selectMember(member);
    await dashBoardPage.navigateToNetworkSetupPage();
  ...
  test //part2
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_019-part2: Validate user is able to create a Journey, receive to receive",
  async (t) => {
  ...
  test //part3
  .meta({ e2e: 'regression', type1: 'usecaseE2E' })
    ("TC_E2E_UC2_019-part3: Posting message using SEND -> RECEIVE, RECEIVE -> RECEIVE",
      async (t) => {
        // Update the subSource and messageuniqueid in requestBody
...
fixture`TC_E2E_UC2_020 - Creating only send to receive journey and posting message - With two stages`
  .page(baseUrl)
  .beforeEach(async (t) => {
    await LoginPage.logInSuccessful();
    await dashBoardPage.selectMember(member);
  ...
  test //part1
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_020-part1: Validate user is able to create a Journey, send to receive",
  async (t) => {
    await dashBoardPage.navigateToNetworkSetupPage();
    await networkSetupPage.navigateToJourneysPage();
    await journeysPage.setJourneyFieldsSaveAndAssert({
  ...
  test //part2
  .meta({ e2e: 'regression', type1: 'usecaseE2E' })
    ("TC_E2E_UC2_020-part2: Posting message using SEND -> RECEIVE",
      async (t) => {
        // Update the subSource and messageuniqueid in requestBody
  ...
  fixture`TC_E2E_UC2_021 - Creating receive to receive journey and posting message - With two stages`
  .page(baseUrl)
  .beforeEach(async (t) => {
    await LoginPage.logInSuccessful();
    await dashBoardPage.selectMember(receiverAsMember);
  ...
  test //part1
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_021-part1: Validate user is able to create a Journey, receive to receive",
  async (t) => {
    await dashBoardPage.navigateToNetworkSetupPage();
    await networkSetupPage.navigateToJourneysPage();
    await journeysPage.setJourneyFieldsSaveAndAssert({
  ...
  test //part2
  .meta({ e2e: 'regression', type1: 'usecaseE2E' })
    ("TC_E2E_UC2_021-part2: Posting message using RECEIVE -> RECEIVE",
      async (t) => {
        // Create a deep copy of the requestBody
     const requestBodyFor021 = JSON.parse(JSON.stringify(requestBody));
        // Update the subSource, messageuniqueid and message type in requestBody
     requestBodyFor021.metadata.messageuniqueid = "receiveToreceive_testcafe_" + Date.now();
     requestBodyFor021.metadata.subSource = t.fixtureCtx.updatedSenderDivision;
     requestBodyFor021.metadata.messageType = "DespatchAdvice";
     
     // Send request
     const sendCompletedMessagRequest = await axios.post(messageURL, requestBodyFor021, { headers: { 'Api-Key': apiKey } }); 
     await t.expect(sendCompletedMessagRequest.status).eql(200); // Asserting response code
     // Save messageuniqueid to fixture context
     const messageuniqueid = requestBodyFor021.metadata.messageuniqueid;
     t.fixtureCtx.messageuniqueid = messageuniqueid;
     ...
     await trafficManagerPage.assertMessageInfo({
       ...
       receiverDivision: requestBody.metadata.subDestination,
       sender:senderName,
       senderDivision:requestBodyFor021.metadata.subSource
   })
