...
fixture`TC_E2E_UC2_001 - Creating send to send journey and posting message - No stages`
  .page(baseUrl)
  .beforeEach(async (t) => {
  ...
  ("TC_E2E_UC2_001-part1  Validating user is able to add the Journey successfully, Direction = SEND and Next direction = SEND", async (t) => {
  ...
  ("TC_E2E_UC2_001-part2: Posting message using send to send journey",  async (t) => {
  ...
  fixture`TC_E2E_UC2_002 - Creating send to receive and receive to offRampEndpoint (JMS.SOLACE.SuccessQueue) journey and posting message - No stages`
  .page(baseUrl)
  .beforeEach(async (t) => {
    await LoginPage.logInSuccessful();
  });
  test //part1
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_002-part1: Validate user is able to create a Journey, send to receive",
  async (t) => {
  ...
  test //part2
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_002-part2: Validate user is able to create a Journey, receive to OffRampEndpoint (JMS.SOLACE.SuccessQueue)",
    async (t) => {
  ...
  test //part3
  .meta({ e2e: 'regression', type1: 'usecaseE2E' })
    ("TC_E2E_UC2_002-part3: Posting message using SEND -> RECEIVE, RECEIVE -> OffRamp (JMS.SOLACE.SuccessQueue)",
      async (t) => {
        try {
          // Update the subSource and messageuniqueid in requestBody
...
fixture`TC_E2E_UC2_003 - Creating send to receive and receive to offRampEndpoint (JMS.SOLACE.FailureQueue) journey and posting message - No stages`
  .page(baseUrl)
  .beforeEach(async (t) => {
    await LoginPage.logInSuccessful();
  });
  test //part1
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_003-part1: Validate user is able to create a Journey, send to receive",
  async (t) => {
    // send to receive journey
    await dashBoardPage.selectMember(member);
    await dashBoardPage.navigateToNetworkSetupPage();
  ...
  test //part2
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_003-part2: Validate user is able to create a Journey, receive to OffRampEndpoint (JMS.SOLACE.FailureQueue)",
    async (t) => {
  ...
  test //part3
  .meta({ e2e: 'regression', type1: 'usecaseE2E' })
    ("TC_E2E_UC2_003-part3: Posting message using SEND -> RECEIVE, RECEIVE -> OffRamp (JMS.SOLACE.FailureQueue)",
      async (t) => {
        // Update the subSource and messageuniqueid in requestBody
...
fixture`TC_E2E_UC2_004 - Creating send to receive and receive to receive journey and posting message - No stages`
  .page(baseUrl)
  .beforeEach(async (t) => {
    await LoginPage.logInSuccessful();
  });
  test //part1
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_004-part1: Validate user is able to create a Journey, send to receive",
  async (t) => {
    // send to receive journey
    await dashBoardPage.selectMember(member);
    await dashBoardPage.navigateToNetworkSetupPage();
  ...
  test //part2
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_004-part2: Validate user is able to create a Journey, receive to receive",
  async (t) => {
  ...
  test //part3
  .meta({ e2e: 'regression', type1: 'usecaseE2E' })
    ("TC_E2E_UC2_004-part3: Posting message using SEND -> RECEIVE, RECEIVE -> RECEIVE",
      async (t) => {
        // Update the subSource and messageuniqueid in requestBody
...
fixture`TC_E2E_UC2_005 - Creating only send to receive journey and posting message - No stages`
  .page(baseUrl)
  .beforeEach(async (t) => {
    await LoginPage.logInSuccessful();
    await dashBoardPage.selectMember(member);
  ...
  test //part1
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_005-part1: Validate user is able to create a Journey, send to receive",
  async (t) => {
    await dashBoardPage.navigateToNetworkSetupPage();
    await networkSetupPage.navigateToJourneysPage();
    await journeysPage.setJourneyFieldsSaveAndAssert({
  ...
  test //part2
  .meta({ e2e: 'regression', type1: 'usecaseE2E' })
    ("TC_E2E_UC2_005-part2: Posting message using SEND -> RECEIVE",
      async (t) => {
        // Update the subSource and messageuniqueid in requestBody
  ...
  fixture`TC_E2E_UC2_006 - Posting message without creating any journey`
  .page(baseUrl)
  .beforeEach(async (t) => {
    await LoginPage.logInSuccessful();
  });
  ...
  test
  .meta({ e2e: 'regression', type1: 'usecaseE2E' })
  ("TC_E2E_UC2_006: Posting message with no journey created",  async (t) => {
    ...
    // Create a deep copy of the requestBody
     ...
     // Update the subSource, messageuniqueid and messageType in requestBody
      requestBodyFor006.metadata.subSource = "SD" + Date.now();
      requestBodyFor006.metadata.messageuniqueid = "testcafe_messageNoJourney" + Date.now();
      requestBodyFor006.metadata.messageType = "MasterdataProductUnit" + Date.now();
    // Send request
    const sendCompletedMessagRequest = await axios.post(messageURL, requestBodyFor006, { headers: { 'Api-Key': apiKey } }); // Include the API key in the headers
    await t.expect(sendCompletedMessagRequest.status).eql(200); // Asserting response code
    // Save messageuniqueid to fixture context
    ...
    await dashBoardPage.selectMember(member);
    await trafficManagerPage.navigateToTrafficManagerPage();
    await trafficManagerPage.seacrhByMessageId(t.fixtureCtx.messageuniqueid);
    await trafficManagerPage.verifyMessageStatus("Error");
    await trafficManagerPage.viewMessage();
    await trafficManagerPage.verifyMessageStatus("Error");
    await trafficManagerPage.assertMessageInfo({
        ...
        receiverDivision: requestBody.metadata.subDestination,
        sender:senderName,
        senderDivision:requestBodyFor006.metadata.subSource
    })
    await trafficManagerPage.validateMessageDetails();
    await trafficManagerPage.validateMessageJourney();
    await trafficManagerPage.validateMessageEvents();
...
//  skipping these tests as stages won't have fail or error tracking points
  fixture`TC_E2E_UC2_007 - Creating send to receive and Posting message via PN ON Ramp IN FAIL - No stages`.skip
  .page(baseUrl)
  .beforeEach(async (t) => {
    await LoginPage.logInSuccessful();
    await dashBoardPage.selectMember(member);
  ...
  test //part1
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_007-part1: Validate user is able to create a Journey, send to receive",
  async (t) => {
    await dashBoardPage.navigateToNetworkSetupPage();
    await networkSetupPage.navigateToJourneysPage();
    await journeysPage.setJourneyFieldsSaveAndAssert({
  ...
  test //part2
  .meta({ e2e: 'regression', type1: 'usecaseE2E' })
  ("TC_E2E_UC2_007-part2: Posting message Posting via PN ON Ramp IN FAILED",  async (t) => {
   // Update the sender division in requestBody
   requestBody.metadata.subSource = t.fixtureCtx.updatedSenderDivision;
   requestBody.metadata.messageuniqueid = "sendToreceiveonlyInFail_testcafe_" + Date.now();
    // Send request
    const sendCompletedMessagRequest = await axios.post(`${messageURL}/fail`, requestBody, { headers: { 'Api-Key': apiKey } }); // Include the API key in the headers
    await t.expect(sendCompletedMessagRequest.status).eql(200); // Asserting response code
    // Save messageuniqueid to fixture context
    const messageuniqueid = requestBody.metadata.messageuniqueid;
    t.fixtureCtx.messageuniqueid = messageuniqueid;
...
//  skipping these tests as stages won't have fail or error tracking points
  fixture`TC_E2E_UC2_008 - Posting message via PN ON Ramp IN FAILED without creating any journey`.skip
  .page(baseUrl)
  .beforeEach(async (t) => {
    await LoginPage.logInSuccessful();
    await dashBoardPage.selectMember(member);
  ...
  test
  .meta({ e2e: 'regression', type1: 'usecaseE2E' })
    ("TC_E2E_UC2_008: Posting message PN ON Ramp IN FAIL with no journey created",
      async (t) => {
        // Update the subSource and messageuniqueid in requestBody
  ...
  fixture`TC_E2E_UC2_009 - Creating receive to receive journey and posting message - No stages`
  .page(baseUrl)
  .beforeEach(async (t) => {
    await LoginPage.logInSuccessful();
    await dashBoardPage.selectMember(receiverAsMember);
  ...
  test //part1
  .meta({  e2e: 'regression', type1: 'usecaseE2E' })(
  "TC_E2E_UC2_009-part1: Validate user is able to create a Journey, receive to receive",
  async (t) => {
    await dashBoardPage.navigateToNetworkSetupPage();
    await networkSetupPage.navigateToJourneysPage();
    await journeysPage.setJourneyFieldsSaveAndAssert({
  ...
  test //part2
  .meta({ e2e: 'regression', type1: 'usecaseE2E' })
    ("TC_E2E_UC2_009-part2: Posting message using RECEIVE -> RECEIVE",
      async (t) => {
        // Create a deep copy of the requestBody
     const requestBodyFor009 = JSON.parse(JSON.stringify(requestBody));

        // Update the subSource, messageuniqueid and message type
     requestBodyFor009.metadata.messageuniqueid = "receiveToreceive_testcafe_" + Date.now();
     requestBodyFor009.metadata.subSource = t.fixtureCtx.updatedSenderDivision;
     requestBodyFor009.metadata.messageType = "DespatchAdvice"; 
     
     // Send request
     const sendCompletedMessagRequest = await axios.post(messageURL, requestBodyFor009, { headers: { 'Api-Key': apiKey } }); // Include the API key in the headers
     await t.expect(sendCompletedMessagRequest.status).eql(200); // Asserting response code
     // Save messageuniqueid to fixture context
     ...
     await trafficManagerPage.assertMessageInfo({
       ...
       receiverDivision: requestBody.metadata.subDestination,
       sender:senderName,
       senderDivision:requestBodyFor009.metadata.subSource
   })
